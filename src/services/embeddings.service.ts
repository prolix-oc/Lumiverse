...
async function toConfigWithStatus(
  ...
  const providerProfiles = cfg.provider_profiles ?? {};
  const profilesWithStatus: Partial<Record<EmbeddingProvider, EmbeddingProviderProfileWithStatus>> = {};
  for (const [provider, profile] of Object.entries(providerProfiles) as Array<[EmbeddingProvider, EmbeddingProviderProfile]>) {
    if (!profile) continue;
    profilesWithStatus[provider] = {
      ...profile,
      has_api_key: await hasEmbeddingSecret(userId, provider),
    };
  }
...
export async function getEmbeddingConfig(userId: string): Promise<EmbeddingConfigWithStatus> {
  ...
  return toConfigWithStatus(ctx.userId, cfg, ctx.inherited || undefined);
}
...
export async function updateEmbeddingConfig(
  ...
  const merged = persistableConfig(mergeEmbeddingConfigUpdate(current, patched));
  ...
  return toConfigWithStatus(userId, merged);
}
...
export async function testEmbeddingConfig(
  ...
  if (ctx.inherited) {
    ...
    return {
      ...
      config: await toConfigWithStatus(ctx.userId, ownerCfg, true),
    };
  }

  const current = readRawEmbeddingConfig(userId);
  const updated = persistableConfig(normalizeConfig({
    ...current,
  ...
  return {
    dimension: first.length,
    config: await toConfigWithStatus(userId, persisted),
  };
}
