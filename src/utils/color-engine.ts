/**
 * Pure color extraction + theme derivation engine.
 *
 * Inspired by how Spotify, Apple Music, and Netflix handle content-aware color:
 *  - Spotify extracts a dominant artwork color and tints the Now Playing
 *    background with it.
 *  - Apple Music builds flowing ambient gradients from saturated artwork
 *    copies + blur, so the UI feels immersed in the cover.
 *  - Netflix keeps its UI chrome static and invests in imagery, which taught
 *    us to keep text/interactive elements on predictable, accessible colors
 *    even when the surrounding palette is dynamic.
 *
 * This engine therefore does four things:
 *  1. Extracts a small, representative palette from raw pixel data.
 *  2. Classifies Vibrant/Muted swatches (Android Palette-style) so callers
 *     can pick an intentional accent instead of whatever happens to be largest.
 *  3. Derives parallel dark- and light-mode readable UI schemes from the same
 *     hue family, so switching modes feels coherent rather than inverted.
 *  4. Produces an ambient gradient pair and a character-aware overlay that
 *     Spindle extensions and the frontend can apply directly.
 *
 * This module has no I/O dependencies (no sharp, no canvas, no DB). It is
 * safe to import from both the backend service and the Spindle worker runtime.
 */
...
export function labToRgb({ L, a, b }: LabColor): RGB {
  const fy = (L + 16) / 116;
  const fx = a / 500 + fy;
  const fz = fy - b / 200;
...
function kMeansPalette(pixels: LabPixel[], k: number, maxIterations = 12): LabColor[] {
  ...
  // Deterministic seeding: pick the most-populated histogram buckets as
  // starting centroids. This avoids the random variance of vanilla k-means.
  const quantStep = 24;
...
function pickFallbackHue(dominant: RGB, average: RGB): number {
  ...
  for (const { color, weight } of candidates) {
    ...
    const chroma = Math.sqrt(lab.a * lab.a + lab.b * lab.b);
    const hsl = rgbToHsl(color.r, color.g, color.b);
    // Near-black/white colors carry a misleading hue; penalize them so we
    // prefer a hue from a color that is actually perceptually colored.
    const lPenalty =
      hsl.l < 10 ? 0.1 :
      hsl.l > 88 ? 0.2 :
...
/**
 * Choose the single hue that will anchor both dark and light surfaces.
 * We prefer a palette color that is both vibrant and reasonably bright; if
 * the image is low-chroma we fall back to the most meaningful hue among
 * dominant/average, and finally to the default hue.
 */
function pickSeedHue(palette: RGB[], dominant: RGB, average: RGB): number {
  // 1. Prefer the most saturated palette color with a usable lightness.
  ...
  // 2. No vibrant swatch: pick the most chromatic color in CIELAB.
  ...
  // 3. Fall back to a hue derived from dominant/average.
  return pickFallbackHue(dominant, average);
}

/**
 * Generate a dark or light surface directly from the seed hue.
 * Instead of mixing an image pixel with a fixed gray, we hold the hue
 * constant and choose a very low-chroma, luminance-safe tone. This is what
 * Material You / Monet does: the surface stays in the image's hue family but
 * never turns into muddy brown or a flash-bang light.
 */
...
function pickAccentBase(colors: RGB[], surface: RGB, seedHue: number): RGB {
  ...
  for (const candidate of candidates) {
    ...
    // Prefer colors that share the seed hue family, but still reward some
    // separation from the surface so the accent doesn't disappear.
    ...
    // Hue match is the strongest signal; separation and vibrancy keep it readable.
...
function deriveReadableScheme(
  ...
  // Surface is now pre-derived from the seed hue (see deriveSurfaceFromSeed),
  // so we only clamp it to our comfort luminance range.
...
function deriveUiSchemes(
  ...
  // Pick a single seed hue for the image. Both dark and light surfaces are
  // derived from this hue, so the theme always stays in the same color family
  // and never collapses into the old fixed blue-gray or muddy brown.
  ...
  // Anchor accent to the same hue family. If the palette has a clear vibrant
  // color, use it; otherwise bias accent selection toward the seed hue.
...
function pickMostVibrant(
  palette: RGB[],
  dominant: RGB,
  regions: ColorExtractionData["regions"],
  flatness: ColorExtractionData["flatness"],
  average: RGB
...
export function deriveCharacterNameVars(
  ...
  const hsl = pickMostVibrant(palette, dominant, regions, flatness, average);
...
export function deriveCharacterOverlay(
  palette: RGB[],
  ui: { dark: ReadableColorScheme; light: ReadableColorScheme }
): CharacterColorOverlay {
  const darkAccent = ui.dark.accent;
  const lightAccent = ui.light.accent;
  const primaryHsl = rgbToHsl(darkAccent.r, darkAccent.g, darkAccent.b);

  const secondarySeed = palette[1] ?? palette[0] ?? darkAccent;
  const secondaryDark = deriveSecondaryTone(secondarySeed, ui.dark.surface, "dark");
  const secondaryLight = deriveSecondaryTone(secondarySeed, ui.light.surface, "light");
  ...
  return {
    ...
    baseColors: {
      ...
      background: rgbToCss(ui.dark.surface),
      text: rgbToCss(ui.dark.text),
    },
    baseColorsLight: {
      primary: rgbToCss(lightAccent),
      secondary: rgbToCss(secondaryLight),
      background: rgbToCss(ui.light.surface),
      text: rgbToCss(ui.light.text),
    },
  };
}
...
/**
 * Extract color palette from raw pixel data.
 */
export function extractColorsFromRawPixels(
  ...
  const regions = {} as ColorExtractionData["regions"];
  const flatness = { full: fullAnalysis.dominant.flatness } as ColorExtractionData["flatness"];
  ...
  // Build a k-means palette from the full image for richer variety, then
  // merge it with the histogram candidates so region accents still show up.
  const { palette: kMeansPalette, populations } = buildKMeansPalette(data, channels, pixelCount, 6);
  ...
  const swatches = classifySwatches(palette, usedFallback ? palette.map(() => 1) : populations);
  const ambient = deriveAmbientGradient(palette, fullAnalysis.dominant.color, fullAnalysis.average, swatches);
  const overlay = deriveCharacterOverlay(palette, ui);
