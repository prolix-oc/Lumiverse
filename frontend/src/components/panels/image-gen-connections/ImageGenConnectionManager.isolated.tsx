...
function resolveActiveConnection(
  ...
  if (selectedId && profiles.some((item) => item.id === selectedId && item.review_required !== true)) return selectedId
  return profiles.find((item) => item.is_default && item.review_required !== true)?.id
    ?? profiles.find((item) => item.review_required !== true)?.id
    ?? null
}
