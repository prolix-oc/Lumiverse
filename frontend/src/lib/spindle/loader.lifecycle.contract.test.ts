...
describe('permission-free drawer and input placements', () => {
  ...
  test('keeps drawer tabs and input actions after a ui_panels permission revoke cleanup pass', async () => {
  ...
  test('keeps a genuinely privileged float widget gated without ui_panels', async () => {
    mockedGrantedPermissions = []
    ...
    const source = `
      export function setup(context) {
        let denied = 0;
        try {
          context.ui.createFloatWidget({ initialPosition: { x: 10, y: 20 } });
        } catch {
          denied += 1;
        }
        try {
          context.ui.requestDockPanel({ title: 'Privileged dock', edge: 'left', size: 240 });
        } catch {
          denied += 1;
        }
    ...
    try {
      await loadPlacementBundle(extensionId, manifest, source)
      expect(loaderGlobals.__privilegedPlacementDenied).toBe(2)
