...
## Agents & Tools intrinsics (not ordinary macros)

Agents & Tools uses a feature-local intrinsic syntax. Do not treat these as entries in the ordinary macro catalog: a scoped intrinsic can run a child provider during prompt assembly, while a normal macro only resolves text.

Producer user block:

```text
{{agent::lore_reviewer::as=review::tools=lore_search_entries,chat_search_history::stream}}
Compare the current scene with the established lore and list only contradictions.
{{/agent}}
```
...
The opening tag must name a configured profile. `as=result_name`, `tools=tool_id,tool_id`, and `stream` are optional, order-independent options; an option cannot be repeated. `tools=` only narrows the profile's checked tools, and `stream` is valid only when that profile allows activity. Profile IDs and result names are case-sensitive and match `[a-z][a-z0-9_]{0,63}`.

Only a whole, directly authored block in a `user` prompt block can execute. The closing tag must match, and scoped agent blocks cannot be nested, recursive, or generated by another macro. Resolve ordinary macros in the task once before the child runs; the child's text is opaque derived data and is not re-evaluated as macros. A named result is readable only after its producer in prompt-block order:

Producer user block:

```text
{{agent::fact_checker::as=facts}}
Check the claim in the current user request.
{{/agent}}
```
...
```text
Use these findings in the next user instruction:
{{agentResult::facts}}
```

Duplicate producers, forward or missing result references, malformed options, a missing profile, or a non-`user` placement fail validation. A required profile failure aborts generation; an optional profile failure records the failure, restores an empty direct value, and binds an empty named result. A task is never silently truncated when it exceeds the feature limit.

When `agentConfig` is explicitly disabled, valid agent blocks and result references are stripped before ordinary macro evaluation so authored child work cannot leak into the main prompt. With no `agentConfig` at all, existing unknown-macro behavior remains unchanged. `agent_delegate` is a separate main-model tool, not an intrinsic and not a macro.
